...
@section('content')
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="dashboard-totals mb-6">
            <div class="total-expenses">
                <h2>Total Expenses for the Month: <span id="totalExpenses">₹0.00</span></h2>
            </div>
            <div class="average-expenses">
                <h2>Average Expenses per Day: <span id="averageExpensesPerDay">₹0.00</span></h2>
            </div>
        </div>

        <div class="charts flex flex-wrap justify-center gap-6">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function fetchDataAndRenderCharts(month, year) {
                fetch(`/dashboard/data?month=${month}&year=${year}`)
                    ...
                    .then(data => {
                        console.log(data.averageExpensesPerDay.toFixed(2))
                        // Update total expenses and average per day in the dashboard
                        document.getElementById('totalExpenses').textContent = `₹${data.totalExpenses.toFixed(2)}`;
                        document.getElementById('averageExpensesPerDay').textContent = `₹${data.averageExpensesPerDay.toFixed(2)}`;
                        ...
                        const pieChartColors = data.pieChartLabels.map((_, index) => colors[index % colors
                            .length]);
